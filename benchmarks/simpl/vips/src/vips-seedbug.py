# This script inserts the seeded bug into VIPS before building it
#
...
# This function comments out the lock and unlock in im_semaphore_upn 
# (file vips-7.20.0/libvips/iofuncs/semaphore.c)
# at lines 87 and 92 respectively

def seedBugInSemaphore():
    print "Adding bug to semaphore.c..."
    cwd = os.getcwd()
...
def placeThrilleAssertsInThreadgroup():
    print "Changing g_assert to thrilleAssertC in threadgroup.c..."
    cwd = os.getcwd()
    libvips_path = os.path.join(cwd, "vips-7.20.0", "libvips")
    assert os.path.exists(libvips_path)
    ...
    for i in range(0, len(threadgroup_c)):
        tmp = threadgroup_c[i].replace("g_assert", "thrilleAssertC")
        threadgroup_c[i] = tmp
...
def main():
    seedBugInSemaphore()
    placeThrilleAssertsInThreadgroup()
