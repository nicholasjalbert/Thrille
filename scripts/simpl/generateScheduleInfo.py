...
def main():
    ...
    for x in fin:
        if "Preemption(" in x:
            ...
            switch_list.append(x.strip())
        if "0x" in x:
            m = addr_regex.search(x)
            assert m is not None
            addr_start = m.start()
            addr_end = m.end()
            addr = x[addr_start:addr_end]
            debuginfo = addr2lineQuery(binary, addr)
            if "??" in debuginfo:
                readable_trace.append(x)
            else:
                readable_trace.append(x.replace(addr, debuginfo))
        else:
            readable_trace.append(x)
