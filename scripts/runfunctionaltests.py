...
def llvmCompile(tr, td, testin, testout):
    ...
    opt_path = os.path.join(tr, "src", "llvm-2.6", "Release", "bin", "opt")
    args = [opt_path, "-load", llvmpass, "-loadstore", "tmp.o", "-f", "-o", "tmppass.o"]
    exit = subprocess.call(args, stdout = fout, stderr = fout)
    os.remove("tmp.o")
    os.remove("iiddump")
    assert exit == 0
    assert os.path.exists(os.path.join(td, "tmppass.o"))
    llc_path = os.path.join(tr, "src", "llvm-2.6", "Release", "bin", "llc")
    args = [llc_path, "-f", "tmppass.o"]
    exit = subprocess.call(args, stdout = fout, stderr = fout)
    os.remove("tmppass.o")
    assert exit == 0
    assert os.path.exists(os.path.join(td, "tmppass.o.s"))
    link = "-L" + bin
    args = [compiler.strip("llvm-"), "tmppass.o.s", "-o", testout, link, \
            "-ldummy", "-lpthread"]
    exit = subprocess.call(args, stdout = fout, stderr = fout)
    #exit = subprocess.call(args)
