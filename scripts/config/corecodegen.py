...
#       from a specification found in the marshalled dict pthreaddict    
# 
...
class ParamNamer:
    ...
    def processParams(self, paramlist):
        ...
        while len(paramlist) > 0:
            ...
            #removes restrict declarations for pointers 
            #since not all compilers support it
...
def genOrigImpl(dict, outfile):
    ...
    namer = ParamNamer()
    for func in dict:
        returntype = mydict[func].pop(0)
        str = returntype + " Originals::" + func + "("
        ...
        str += "    if (_" + func + "== NULL) {\n"
        str += "        printf(\"Thrille: ERROR: original " + func  
        str +=  " is NULL\\n\");\n"
        if returntype == "void":
            str += "        return;"
        elif returntype == "int":
            ...
            str += "        pthread_t * tid = new pthread_t;\n"
            str += "        return *tid"
        elif returntype == "unsigned int":
            str += "        return 0"
        
        else:
            print "ERROR, returntype", returntype, "not handled"
            print "for func", func
            sys.exit(1);
...
def genLibInterposImpl(dict, outfile):
    ...
    for func in dict:
        ...
        if "sched_yield" in func:
            str += "throw () "
        str += "{\n"
        str += "    if (! Handler::pth_is_initialized) {\n"
        str += "        printf(\"" + func + " is calling pth_start\\n\");\n"
        str += "        pth_start();\n"
        str += "        if (! Handler::pth_is_initialized) {\n"
        str += "            printf(\"" + func + " initialization fail\\n\");\n"
        str += "            _Exit(UNRECOVERABLE_ERROR);\n"
        str += "        }\n    }\n\n"
